...
#define OMEGA_WIRE_DECLARE_WIRE_READWRITE(r_type,w_type,fn_type) \
	inline void wire_read(const wchar_t* pszName, Remoting::IMessage* pMessage, r_type& val) \
	{ \
		val = pMessage->OMEGA_CONCAT(Read,fn_type)(pszName); \
	} \
	inline void wire_read(const wchar_t* pszName, IMarshaller*, Remoting::IMessage* pMessage, r_type* pVals, uint32_t count) \
	{ \
		pMessage->OMEGA_CONCAT(Read,OMEGA_CONCAT_R(fn_type,s))(pszName,count,pVals); \
	} \
	inline void wire_write(const wchar_t* pszName, Remoting::IMessage* pMessage, w_type val) \
	{ \
		pMessage->OMEGA_CONCAT(Write,fn_type)(pszName,val); \
	} \
	inline void wire_write(const wchar_t* pszName, IMarshaller*, Remoting::IMessage* pMessage, const r_type* pVals, uint32_t count) \
	{ \
...
namespace Omega
{
	namespace System
	{
		namespace MetaInfo
		{
			...
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(byte_t,byte_t,Byte)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(bool_t,bool_t,Boolean)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(int16_t,int16_t,Int16)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(uint16_t,uint16_t,UInt16)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(int32_t,int32_t,Int32)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(uint32_t,uint32_t,UInt32)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(int64_t,const int64_t&,Int64)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(uint64_t,const uint64_t&,UInt64)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(float4_t,float4_t,Float4)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(float8_t,const float8_t&,Float8)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(guid_t,const guid_t&,Guid)
			OMEGA_WIRE_DECLARE_WIRE_READWRITE(string_t,const string_t&,String)
