...
case $target in
	...
	all|build|depend|install|tall)
		make_all $target $RELEASE_FLAG $VERBOSE_FLAG $* 
	;;
	# clean all the sub projects
	clean|distclean)
		make_all clean $RELEASE_FLAG $VERBOSE_FLAG $* ;
		make_project "OOCore" distclean $RELEASE_FLAG $VERBOSE_FLAG $* ;
	;;	
	# build the core
	oocore|ooserver|oouser|core|server|user)
		case $target in 
		oocore|core) 	 make_project "OOCore" $RELEASE_FLAG $VERBOSE_FLAG $* ;;
		# build the sandbox process
		oouser|user) 	 make_project "OOSvrUsr" $RELEASE_FLAG $VERBOSE_FLAG $* ;;
		ooserver|server) make_project "OOServer" $RELEASE_FLAG $VERBOSE_FLAG $* ;;
