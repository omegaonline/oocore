...
{
	ACE_Time_Value spin(0,100);

	bool bRet = false;
	if (timeout)
	{
		ACE_Time_Value val(timeout / 1000, (timeout % 1000) * 1000);
		ACE_Countdown_Time countdown(&val);

		do
		{
			bRet = (m_completion.wait(&spin,0) != -1);
			countdown.update();
		} while (!bRet && val != ACE_Time_Value::zero && !Omega::HandleRequest(val.msec()));
	}
	else
	{
		do
