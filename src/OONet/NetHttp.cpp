...
string_t User::HttpProtocolHandler::FindProxy(const string_t& strURL, const string_t& strProtocol)
{
	...
	OOBase::SmartPtr<Db::Statement> ptrStmt = m_db.prepare_statement("SELECT ProxyURL FROM Proxy WHERE URL = %Q;",strURL.ToUTF8().c_str());
	
	if (ptrStmt && ptrStmt->step() == SQLITE_ROW)
