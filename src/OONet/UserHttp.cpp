...
namespace User
{
	...
	private:
		...
		bool                             m_bClose;
		bool                             m_bHasLength;
		OOBase::AtomicInt<unsigned long> m_content_length;
...
void User::Manager::open_http(ACE_InputCDR& request, ACE_OutputCDR& response)
{
	...
	else
	{
		...
		{
			OOBase::SmartPtr<HttpConnection> new_conn(pConn);

			// Insert into map
			OOBase::Guard<OOBase::RWMutex> guard(m_http_lock);
			
			try
			{
				// Okay, got a new HTTP connection
				m_mapHttpConnections.insert(std::map<uint16_t,OOBase::SmartPtr<HttpConnection> >::value_type(conn_id,new_conn));
			}
			catch (std::exception&)
			{
				err = EINVAL;
...
void User::Manager::recv_http(ACE_InputCDR& request)
{
	...
	if (err == 0)
	{
		...
		OOBase::SmartPtr<HttpConnection> conn;

		// Lookup connection
		try
		{
			OOBase::ReadGuard<OOBase::RWMutex> guard(m_http_lock);

			// Okay, got a new HTTP connection
			std::map<uint16_t,OOBase::SmartPtr<HttpConnection> >::iterator i=m_mapHttpConnections.find(conn_id);
...
{
	std::map<uint16_t,OOBase::SmartPtr<HttpConnection> > conns;
