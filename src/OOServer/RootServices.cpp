...
namespace
{
...
#endif
	{
		...
		// Send to service
		err = ptrSocket->send(OOBase::uint32_t(strSocketName.length()));
		if (!err)
			ptrSocket->send(strSocketName.c_str(),OOBase::uint32_t(strSocketName.length()),err);
		if (!err)
#if defined(_WIN32)
	...
	void enum_sockets(OOBase::SmartPtr<Db::Hive> ptrRegistry, const OOBase::LocalString& strName, OOBase::RefPtr<OOBase::Socket> ptrSocket, const OOBase::int64_t& key)
	{
		...
		ptrSocket->send(OOBase::uint32_t(0));
		if (err2)
...
bool Root::Manager::start_services()
{
	...
	OOBase::Queue<OOBase::int64_t,OOBase::AllocatorInstance> queueKeys(allocator);
...
void Root::Manager::start_service(OOBase::uint32_t channel_id, OOBase::CDRStream& request, OOBase::CDRStream& response)
{
...
void Root::Manager::stop_service(OOBase::uint32_t channel_id, OOBase::CDRStream& request, OOBase::CDRStream& response)
{
	// Check for permissions
	OOServer::RootErrCode_t err;
	//if (!m_sandbox_channel || channel_id == m_sandbox_channel)
...
void Root::Manager::service_is_running(OOBase::uint32_t channel_id, OOBase::CDRStream& request, OOBase::CDRStream& response)
{
	// Check for permissions
	OOServer::RootErrCode_t err;
	//if (!m_sandbox_channel || channel_id == m_sandbox_channel)
...
void Root::Manager::service_list_running(OOBase::uint32_t channel_id, OOBase::CDRStream& response)
{
	// Check for permissions
	OOServer::RootErrCode_t err;
	//if (!m_sandbox_channel || channel_id == m_sandbox_channel)
